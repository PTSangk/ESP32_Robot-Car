...
while cap.isOpened():
    ...
    if results.pose_landmarks: 
        ...
        #print(NOSE_Visibility)
        print(f"{timestamp} >> RIGHT_SHOULDER_Visibility: {RIGHT_SHOULDER_Visibility}")
        ...
        #print("Average")
        Average = (NOSE_Visibility + RIGHT_SHOULDER_Visibility + RIGHT_HIP_Visibility ) / 3
        #print(Average)
        ...
        if Average >= 0.9:  #classify is person in screen?
            ...
            if left_shoulder_x < left_threshold:
                ...
                rotateMotorWithDelay(motor_pins, 0.01, GPIO.LOW, GPIO.HIGH, GPIO.HIGH, GPIO.LOW)  # หมุนซ้ายเป็นระยะเวลา 0.1 วินาที
            elif right_shoulder_x > right_threshold:
                print(f"{timestamp} >> Person is on the right")
                rotateMotorWithDelay(motor_pins, 0.01, GPIO.HIGH, GPIO.LOW, GPIO.LOW, GPIO.HIGH)  # หมุนขวาเป็นระยะเวลา 0.1 วินาที
            elif distance > 2.0:
